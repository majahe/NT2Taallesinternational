...
// First, let's update the status ENUM to include 'Scheduled'
...
if (isset($_POST['schedule_course'])) {
  ...
  // Debug: Let's see what we're trying to update
  error_log("Scheduling course for ID: $registration_id");
  error_log("Date: $course_date, Time: $course_time");
  error_log("Instructor: $instructor, Location: $location");
  ...
  if ($stmt) {
    $result = $stmt->bind_param("sssssi", $course_date, $course_time, $instructor, $location, $notes, $registration_id);
    if ($result) {
      ...
      if ($execute_result) {
        ...
        $stmt->close();
        header("Location: planning_fixed.php?success=1");
        exit;
      } else {
...
<body class="dashboard-body">
  <header class="admin-header">
    <h1>📅 Course Planning (Fixed Version)</h1>
  ...
  <div class="planning-container">
    <?php if(isset($_GET['success'])): ?>
      <div class="success-message">
        ✅ Course scheduled successfully! The student status has been updated to "Scheduled".
      </div>
    <?php endif; ?>
